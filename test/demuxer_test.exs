    ...
    def handle_notification({:new_channel, {track_id, details}}, :demuxer, _context, state) do
      case details.codec do
        :opus ->
          children = %{
<<<<<<< HEAD
            # TODO the opus stream generated by the demuxer is playable but the opus file created by the sink is not because it is not ogg encapsulated
            # ogg_plugin doesn't work and needs to be fixed
            # at the moment the only way to verify the opus stream is correct is to pipe it to portaudio though that doesn't work with > 2 channels
            #
=======
>>>>>>> parent of e13d6a2 (pass ivf option through demuxer track details)
...
<<<<<<< HEAD
              # TODO maybe change IVF.Serializer so that these values can be passed via caps
              width: details.width,
